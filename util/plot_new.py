...
def onmf_to_csv(features, gene_name, file_path, thres=0.01):
    ...
    rec_max_gene = 0  # Initialize a variable to record the maximum number of genes observed.
    with open(file_path, 'w', newline='') as file:
        write = csv.writer(file)
        for spin in range(num_spin):
            # Count the number of genes that exceed the threshold for the current spin
            ...
            cur_line.insert(0, spin)  # Insert the spin number at the beginning of the row
...
def gene_program_decomposition(onmf_summary,
    ...
    # Identifying the gene modules by selecting the highest feature component for each gene
    ...
    for ind in range(num_spin):
        ...
        cur_gene = gene_in_mod[np.argsort(- features[ind, gene_in_mod])[: num_gene_select]]
...
def temporary_spin_name(csv_file, num_gene: int = 4):
    ...
    # Create spin names by joining the first 6 elements (or less if not available) of each column in the dataframe
...
def format_label(label):
    ...
    parts = label.split('_')
    i = 0
    # Loop through the parts and insert a newline character after every second part
    while i < len(parts) - 1:
        if i % 2 == 1:
            parts[i] = parts[i] + '\n'
        i += 1
